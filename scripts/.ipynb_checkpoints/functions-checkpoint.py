...
def getResponses(folder, data, R=1, directory = "final"):
  if R == 1:
    res = "responses.csv"
    responses = pd.read_csv("../embeddings/" + directory + "/" +folder+data+res, index_col=0).T #reversed
    savePath = "../results/" + directory + "/" +folder+"reversed/"
    ...
    savePath = "../results/" + directory + "/" +folder+"nonReversed/"
    items_ids = responses.columns
    items = responses.iloc[0,:].values
    if "item" in responses or "item" in responses.index:
      responses = responses.drop(["item"], axis=0)
...
def getEmbeddings(folder, data, embeddings, responses, directory = "final"):
  #read in embeddings, order them like the response data and save in vector
  embeddings_df = pd.read_csv("../embeddings/" + directory + "/" + folder + data +  embeddings,index_col=0)
  ...
  X_pca = pd.DataFrame(PCA(0.95, random_state=0).fit_transform(X_stand),index=question_ids) 
...
def getData(m, responses, X_pca_stand, folder, data, directory = "final"):
  ...
  constructs_list = pd.read_csv("../embeddings/" + directory + "/" + folder + data + "questions_text.csv", encoding = 'utf-8',index_col=0)
...
def compareModels(verbose = 0, directory = "final"):
    for l in ['sentencebert']: #go through the three embeddings
      for k in range(5):                       # calculate for each prediction model: Ridge, 
          ...
          ############################ choose output here ##############################
          Path("../results/" + directory + "/" + d.upper() + "/nonReversed/").mkdir(parents=True, exist_ok = True)
          Path("../results/" + directory + "/" + d.upper() + "/reversed/").mkdir(parents=True, exist_ok = True)
          ...
          data_q, constructs_list, list_par, constrAssigned = getData(m, responses, X_pca_stand, folder, data, directory)
...
def modelPerformance(m=0, par=1, d="BIG5", e="sentencebert", verbose=0, directory = "final"):
...
def regCorr(target_list, n_steps, model_data, directory = "final"): #also calculate model data in the same way
    responses = pd.read_csv('../embeddings/' + directory + "/" + "BIG5/" + "big5_responses_nonReversed.csv", index_col=0) #load actual participant responses
...
def contextAccuracy(context="construct", i='Big5', directory = "final"):
  ...
  if context=="keys":
    ...
    #import required data and labels:
    embeddings_df = pd.read_csv("../embeddings/" + directory + "/" + folder + data + embeddings,index_col=0)
    constructs = pd.read_csv("../embeddings/" + directory + "/" + folder + data + "questions_text.csv", encoding = 'utf-8',index_col=0)
...
def contextAccuracy_constr(context="construct", i='Big5', directory = "final"):
...
def compareModelsConstructs(verbose = 0):
    for  l in ['sentencebert', "word2vec", "liwc"]: #go through the three embeddings
      for k in range(1,4):                          #calculate for each prediction model
        ...
        responses, savePath, items, _ = getResponses(folder, data, 2, directory)
